...
| **Auth / guest** | Local accounts with **username or work email**; **Forgot password** by email (Resend); field-level validation on register; login/register rate limits; signed-in chat burst limits; guest daily cap (cookie + IP) + burst; guest vision up to 2 images when File upload / File import is on; admin middleware by session role |
...
| Feature | Status | Notes |
|---------|--------|-------|
| Forgot password | `done` | Sign-in link, Resend mail, unknown-account message, username-only accounts need an admin |
